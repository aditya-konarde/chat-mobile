...
# Install dependencies for mattermost app
yum -y update && \
yum -y install epel-release && \
yum -y install nodejs zlib.i686 libstdc++.i686 make git android-tools.x86_64 java-1.8.0-openjdk-devel java-1.8.0-openjdk-headless wget unzip && \
yum clean all
...
MATTERMOST_BUILD_VERSION="1.4.1" && \
cd /opt/ && \
wget https://github.com/mattermost/mattermost-mobile/archive/v1.4.1.tar.gz && \
tar xvf v$MATTERMOST_BUILD_VERSION.tar.gz;

# Finally build the app
cd /opt/mattermost-mobile-$MATTERMOST_BUILD_VERSION && \
yarn install && \
make
